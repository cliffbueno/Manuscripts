# DADA2 analysis of Herbicide Field 3/4 Peru Microcosm 2 16S Data
...
data.fp <- "~/Desktop/CU/2Research/Herbicide/Fall2019/16S"
...
barcode.fp <- file.path(data.fp, "Barcodes_16S.txt")
map.fp <- file.path(data.fp, "MappingFile16S.txt")
...
project.fp <- "/Users/cliffbuenodemesquita/Desktop/CU/2Research/Herbicide/Fall2019/16S"
...
# My plots look pretty good, mostly above 30, no big dropoffs. Small steady decline towards end in both reads. Just trim off last 10 in each.
filt_out <- filterAndTrim(fwd=file.path(subF.fp, fastqFs), filt=file.path(filtpathF, fastqFs), rev=file.path(subR.fp, fastqRs), filt.rev=file.path(filtpathR, fastqRs), truncLen=c(240,240), maxEE=1, truncQ=11, maxN=0, rm.phix=TRUE, compress=TRUE, verbose=TRUE, multithread=TRUE)
...
seqtab.nochim <- readRDS("../Herbicide/Fall2019/16S/03_tabletax/seqtab_final.rds")

# Assign taxonomy
tax <- assignTaxonomy(seqtab.nochim, "/Users/cliffbuenodemesquita/Desktop/db_files/silva_nr_v132_train_set.fa", tryRC = TRUE, multithread=TRUE)
...
library(mctoolsr)
library(biomformat)
tax_table_fp = '/Users/cliffbuenodemesquita/Desktop/CU/2Research/Herbicide/Fall2019/16S/03_tabletax/seqtab_wTax_mctoolsr.txt'
map_fp = '/Users/cliffbuenodemesquita/Desktop/CU/2Research/Herbicide/Fall2019/16S/MappingFile16S.txt'
...
# First export and examine taxa in controls and decide to remove anything
write.csv(input_filt$data_loaded, "ASV_table_unrarefied.csv")
# May want to remove ESV_130 for Chatfield Stuff. Peru stuff fine.
...
input_filt_rare_PM <- single_rarefy(input = input_filt, depth = 12556)
...
write_biom(pb, "Peru_ASV_table_rare_16S.biom")
write.csv(input_filt_rare_PM$data_loaded, "Peru_ASV_table_rare_16S.csv")

# Chatfield (need to do)
input_filt_rare <- single_rarefy(input = input_filt, depth = 5111) # CHANGE ME to desired depth.
# Remove other blank sample
input_filt_rare = filter_data(input_filt_rare, 'Description', filter_vals = 'Blank')
# Check - only samples we want and all rarefied
